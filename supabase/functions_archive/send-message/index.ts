...
serve(async (req) => {
  ...
  try {
    ...
    if (!organization_id || !phone || !message) {
      throw new Error(
        "Dados incompletos: organization_id, phone, message são obrigatórios.",
      );
    }
    ...
    if (leadData) {
      ...
      // For now, assuming LiveChat usage, the lead should exist.
      // If used for campaigns on raw numbers, we might need to create one.
      // Let's create a minimal lead to ensure history is saved.
    ...
    let providerUsed = "";
    const whatsappConfig = Array.isArray(orgData.whatsapp_config)
      ? orgData.whatsapp_config[0]
      : orgData.whatsapp_config;
